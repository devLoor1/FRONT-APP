    ...
    <View style={styles.container}>
      <View style={styles.head}>
        <View>
          ...
          <BlurValues value={resume?.investedAmount.toFixed(2) || '0'} />
        </View>
      </View>
      <View style={styles.list}>
        <View style={{ flex: 0.33 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
            <Text style={{ ...styles.itemValue }}>
              R${' '}
              {showBalance ? CommonMask.currency(resume?.expectedProfit?.toFixed(2) || '0') : '-'}
            </Text>
        ...
        <View style={{ flex: 0.33, alignItems: 'center' }}>
          ...
          <View
            ...
            <Text style={styles.itemValue}>
              R${' '}
              {showBalance
                ? CommonMask.currency(resume?.totalValueToReceive.toFixed(2) || '0')
                : '-'}
            </Text>
            <ArrowUp width={12} height={12} color={theme.customColors.risk.default} />
          </View>
        ...
        <View style={{ flex: 0.33, alignItems: 'flex-end' }}>
          ...
          <Text style={[styles.itemValue, { color: theme.customColors.neutrals[500] }]}>
            {showBalance ? CommonMask.percent(resume?.tir?.toFixed(2).toString() || '0') : '-'}%
          </Text>
        </View>
      </View>
      <ModalDefault
